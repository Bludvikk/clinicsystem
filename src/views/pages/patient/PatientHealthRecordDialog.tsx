...
import { useMedicationsStore, usePatientFormStore } from '@/utils/patient.store';
...
const PatientHealthRecordDialog = () => {
  ...
  const { id, onClosing, isSaving, showDialog, dialogTitle, steps, activeStep, setActiveStep } = usePatientFormStore(
    state => state
  );

  const { medications, onAdd, onDelete, onClear, onReplaceAll } = useMedicationsStore();
  ...
  const PATIENT_REVIEW_PANELS: Record<(typeof PATIENT_HEALTH_RECORD_PANELS)[number], ListItemTextType[]> = {
    PersonalInformation: [
      {
      ...
      },
      {
        listItemTextAttribute: {
          primary: 'Occupation',
          primaryTypographyProps: { fontWeight: 'bold' },
          secondary: referencesData?.find(ref => ref.id === getValues('occupationId'))?.name,
          secondaryTypographyProps: { variant: 'body2' }
        },
        gridAttribute: { xs: 6, sm: 4, md: 3, lg: 2 }
      },
  ...
  const medicationOnSubmit: SubmitHandler<IMedication> = (data: IMedication) => {
    medicationReset();
    onAdd(data);
  };

  const postPatientOnSubmit: SubmitHandler<IAddPatient> = (data: IAddPatient) => {
    if (id) {
      putPatientMutate(
        { id, ...data },
        {
  ...
  useEffect(() => {
    if (id && patientData) {
      ...
      const JSONWithDatesData = parseJSONWithDates(JSON.stringify(data)) as IAddPatient; // cast it to your desired type to have auto-complete
        ...
        return (
          <form key={0} id='form-step-0' onSubmit={handleSubmit(handleNext)}>
            <Grid container spacing={5}>
              {PATIENT_HEALTH_RECORD_FIELDS['PersonalInformation'].map((obj, i) => (
          ...
          <form key={2} id='form-step-2' onSubmit={handleSubmit(handleNext)}>
            <Grid container spacing={6}>
              <Grid container item spacing={5} xs={12} md={6}>
                {PATIENT_HEALTH_RECORD_FIELDS['PersonalHistory'].map((obj, i) => (
                  <Grid item key={obj.dbField} {...obj.extendedProps?.gridAttribute}>
                    <FormObjectComponent
                      key={i}
                      objFieldProp={obj}
                      ...
                      {medications &&
                        ...
                        medications.map((medication, i) => (
                          <ListItem
                            key={i}
                            secondaryAction={
                              <IconButton type='button' color='secondary' onClick={() => onDelete(i)}>
      ...
      case 3:
        return (
          <form key={3} id='form-step-3' onSubmit={handleSubmit(handleNext)}>
            <Grid container spacing={5}>
              {PATIENT_HEALTH_RECORD_FIELDS['PastMedicalHistory'].map((obj, i) => (
                <Grid item key={obj.dbField} {...obj.extendedProps?.gridAttribute}>
                  <FormObjectComponent
                    key={i}
                    objFieldProp={obj}
    ...
    if (activeStep === steps.length) {
      ...
      return (
        <Grid container>
          ...
          <Grid mt={5} item xs={12} sx={{ display: 'flex', justifyContent: 'end' }}>
            {activeStep === 0 ? (
              <Button size='large' onClick={() => handleClose()} sx={{ mr: 3 }}>
                Cancel
              </Button>
            ...
            {activeStep === steps.length - 1 ? (
              ...
              <Button form={`form-step-${activeStep}`} size='large' variant='contained' type='submit'>
                Next
              </Button>
            )}
          </Grid>
